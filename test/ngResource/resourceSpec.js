...
describe('resource', function() {
  ...
  describe("wrt normal usage", function() {
  ...
  describe('wrt cancelling requests', function() {
    var httpSpy;
    var $httpBackend;
    var $resource;
    ...
    it('should prevent non-numeric timeouts in actions and log a $debug message',
      inject(function($log, $q) {
        ...
        expect(httpSpy).toHaveBeenCalledOnce();
        expect(httpSpy.calls[0].args[0].timeout).not.toBe(promise);
    ...
    it('should cancel the request that created the given resource instance/collection when calling `cancelRequest`', function() {
      var mockCC = {};
      $httpBackend.whenGET('/CreditCard').respond(mockCC);
    ...
    it('should not do anything when calling `cancelRequest` if the request has already completed', function() {
      var mockCC = {a: 'a', b: 'b'};
      $httpBackend.whenGET('/CreditCard').respond(mockCC);
      ...
      CreditCard.cancelRequest(cc);
      expect(cc).toEqual(jasmine.objectContaining(mockCC));
    });

    it('should not do anything when calling `cancelRequest` if the request has already been cancelled', function() {
      var mockCC = {a: 'a', b: 'b'};
      $httpBackend.whenGET('/CreditCard').respond(mockCC);
